...
public class FileSectionComponentValidator extends ValidatorComponentImpl {
  ...
  private ReporterDetails validateCSIP76() {
    ...
    for (MetsType.FileSec.FileGrp grp : fileGrp) {
      ...
      for (FileType file : files) {
        List<FileType.FLocat> flocat = file.getFLocat();
        if (flocat == null) {
          return new ReporterDetails(Constants.VALIDATION_REPORT_HEADER_CSIP_VERSION,"mets/fileSec/fileGrp/file/FLocat can't be null!", false,false);
        } else {
          if (flocat.size() != 1) {
            return new ReporterDetails(Constants.VALIDATION_REPORT_HEADER_CSIP_VERSION,"mets/fileSec/fileGrp/file/FLocat cannot have more than one", false,false);
          }
        }
      }
    }
      ...
      for (FileType file : files) {
        ...
        if (flocat == null) {
          ...
          for (FileType.FLocat floc : flocat) {
            ...
            if (loctype == null) {
              return new ReporterDetails(Constants.VALIDATION_REPORT_HEADER_CSIP_VERSION,"mets/fileSec/fileGrp/file/FLocat[@LOCTYPE=’URL’] can't be null!", false,false);
            } else {
              if (!loctype.equals("URL")) {
                return new ReporterDetails(Constants.VALIDATION_REPORT_HEADER_CSIP_VERSION,"mets/fileSec/fileGrp/file/FLocat[@LOCTYPE=’URL’] value has to be URL",
                  false,false);
              }
            }
          }
        ...
        if (flocat == null) {
          ...
          for (FileType.FLocat floc : flocat) {
            ...
            if (href != null) {
              ...
              return new ReporterDetails(Constants.VALIDATION_REPORT_HEADER_CSIP_VERSION,"mets/fileSec/fileGrp/file/FLocat/@xlink:href can't be null!", false,false);
            }
          }
        }
      }
    ...
    for (MetsType.FileSec.FileGrp fileGrp : fileGrps) {
      ...
      if (count != files.size()) {
        return new ReporterDetails(Constants.VALIDATION_REPORT_HEADER_SIP_VERSION,
          "can add fileSec/fileGrp/file/@sip:FILEFORMATNAME  if the MIMETYPE is not sufficient for the purposes of processing the information package.",
          false, false);
      }
    }
    return new ReporterDetails();
  ...
  private ReporterDetails validateSIP33() {
    ...
    for (MetsType.FileSec.FileGrp fileGrp : fileGrps) {
      ...
      if (count != files.size()) {
        return new ReporterDetails(Constants.VALIDATION_REPORT_HEADER_SIP_VERSION,
          "can add fileSec/fileGrp/file/@sip:FILEFORMATVERSION The version of the file format when the use of PREMIS has not been agreed upon in the submission agreement.",
          false, false);
      }
    }
    return new ReporterDetails();
  ...
  private ReporterDetails validateSIP34() {
    ...
    for (MetsType.FileSec.FileGrp fileGrp : fileGrps) {
      ...
      if (count != files.size()) {
        return new ReporterDetails(Constants.VALIDATION_REPORT_HEADER_SIP_VERSION,
          "can add fileSec/fileGrp/file/@sip:FILEFORMATREGISTRY The name of the format registry used to identify the file format when the use of PREMIS has not been agreed upon in the submission agreement.",
          false, false);
      }
    }
    return new ReporterDetails();
  ...
  private ReporterDetails validateSIP35() {
    ...
    for (MetsType.FileSec.FileGrp fileGrp : fileGrps) {
      ...
      if (count != files.size()) {
        return new ReporterDetails(Constants.VALIDATION_REPORT_HEADER_SIP_VERSION,
          "can add fileSec/fileGrp/file/@sip:FILEFORMATKEY Key of the file format in the registry when use of PREMIS has not been agreed upon in the submission agreement.",
          false, false);
      }
    }
    return new ReporterDetails();
