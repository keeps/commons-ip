    ...
    private ReporterDetails validateCSIP35(){
        for(AmdSecType a: amdSec){
            ...
            for(MdSecType md: digiprov){
                MdSecType.MdRef mdRef = md.getMdRef();
                if(mdRef == null){
                    return new ReporterDetails(Constants.VALIDATION_REPORT_HEADER_CSIP_VERSION,"mets/amdSec/digiprovMD/mdRef is the reference to the digital provenance metadata file",false,false);
                }
            }
        }
        return new ReporterDetails();
                ...
                if(mdRef != null){
                    ...
                    if(loctype != null) {
                        if(!loctype.equals("URL")){
                            return new ReporterDetails(Constants.VALIDATION_REPORT_HEADER_CSIP_VERSION,"mets/amdSec/digiprovMD/mdRef[@LOCTYPE='URL'] value must be URL",false,false);
                        }
                    }
                    else{
                        return new ReporterDetails(Constants.VALIDATION_REPORT_HEADER_CSIP_VERSION,"mets/amdSec/digiprovMD/mdRef[@LOCTYPE='URL'] can't be null",false,false);
                    }
                }
            }
        }
