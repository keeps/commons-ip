    ...
    try {
      ...
      if (validateOptions(commandLine)) {
        ...
        final String metadataSchema = commandLine
          .getOptionValue(CLIConstants.CLI_CREATE_LONG_OPTION_METADATA_SCHEMA_WITHOUT_IDENT) == null
          ? commandLine.getOptionValue(CLIConstants.CLI_CREATE_SHORT_OPTION_METADATA_SCHEMA_WITHOUT_IDENT)
          : commandLine.getOptionValue(CLIConstants.CLI_CREATE_LONG_OPTION_METADATA_SCHEMA_WITHOUT_IDENT);
        ...
        try {
          final Path eark2SIP = SipCreatorUtils.createEARK2SIP(metadata,
            metadataSchema, representation, targetOnly, sipID, ancestors, documentation,
